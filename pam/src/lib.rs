...
use usertree_spawner_protocol::{PamReply, PamRequest};
...
unsafe fn parse_options(
    ...
    for index in 0..argc as isize {
        ...
        let Some(value) = argument.strip_prefix("timeout=") else {
            return Err("unsupported pam_usertree_spawner option");
        };
        let seconds: u64 = value
            .parse()
            .map_err(|_| "invalid pam_usertree_spawner timeout")?;
        if !(1..=600).contains(&seconds) {
            return Err("pam_usertree_spawner timeout must be between 1 and 600 seconds");
        }
