...
const LOCK_FILE: &str = "/run/lock/usertree-spawner-pam.lock";
const MANAGED_LINE: &str = "session    required   pam_usertree_spawner.so\n";

fn main() {
    if let Err(error) = run() {
        eprintln!("usertree-spawner-pam: {error:#}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<()> {
    ...
    let command = arguments
        ...
        .context("usage: usertree-spawner-pam enable|disable|status")?;
    if arguments.next().is_some() {
        bail!("usage: usertree-spawner-pam enable|disable|status");
    }
    ...
    match command.as_str() {
        "enable" => {
            ...
            println!("enabled pam_usertree_spawner in {SYSTEM_LOGIN}");
        }
        "disable" => {
            require_root()?;
            let _lock = Lock::acquire()?;
            update(Path::new(SYSTEM_LOGIN), Action::Disable)?;
            println!("disabled pam_usertree_spawner in {SYSTEM_LOGIN}");
        }
        "status" => {
            let source =
        ...
        _ => bail!("usage: usertree-spawner-pam enable|disable|status"),
    }
    Ok(())
}
...
fn atomic_replace(path: &Path, contents: &[u8], metadata: &fs::Metadata) -> Result<()> {
    ...
    let temporary = parent.join(format!(".{name}.usertree-spawner-{}", std::process::id()));
...
fn inspect(source: &str) -> Result<PamState> {
    ...
    let usersv_lines: Vec<_> = lines
        ...
        .filter_map(|(index, line)| {
            (module_name(line) == Some("pam_usertree_spawner.so")).then_some((index, *line))
        })
        .collect();
    match usersv_lines.as_slice() {
        ...
        _ => {
            bail!(
                "unexpected pam_usertree_spawner configuration; refusing to modify {SYSTEM_LOGIN}"
            )
        }
    }
}
...
fn is_managed_line(line: &str) -> bool {
    let fields: Vec<_> = line.split_whitespace().collect();
    fields == ["session", "required", "pam_usertree_spawner.so"]
}
...
mod tests {
    ...
    fn refuses_unmanaged_usersv_configuration() {
        let source = format!("{ARTIX}session optional pam_usertree_spawner.so timeout=60\n");
        assert!(inspect(&source).is_err());
        assert!(enable(&source).is_err());
        assert!(disable(&source).is_err());

        let misplaced = format!("session required pam_usertree_spawner.so\n{ARTIX}");
