...
use usertree_spawner_core::{
    account::{Account, resolve_uid},
    config::{Config, DEFAULT_CONFIG_PATH},
    ipc::SeqPacket,
    security::verify_root_owned_file,
};
use usertree_spawner_protocol::{DaemonToHelper, HelperToDaemon, ProcessStatus, ReadinessFrame};

fn main() {
    if let Err(error) = run() {
        eprintln!("usertree-spawner-supervisor: fatal: {error:#}");
        std::process::exit(1);
    }
}
    ...
    fn stop_manager(&mut self) -> Result<()> {
            ...
            .spawn_stop(&self.account, &self.environment, manager.child.pid())
        {
            Ok(mut stop) => match stop.wait_timeout(self.config.graceful_stop_timeout())? {
                ...
                Some(status) => {
                    eprintln!("usertree-spawner-supervisor: backend stop hook failed with {status}")
                }
                None => {
                    eprintln!("usertree-spawner-supervisor: backend stop hook timed out");
            ...
            Err(error) => {
                eprintln!(
                    "usertree-spawner-supervisor: cannot execute backend stop hook: {error:#}"
                )
            }
        }
