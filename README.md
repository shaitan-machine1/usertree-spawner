...
`usertree-spawner` is an elogind-specific launcher and lifecycle supervisor for
...
[`s6-user`](https://github.com/shaitan-machine1/s6-user) policy wrapper and does
not claim to be an official s6 per-user implementation.

## Packages

- **usertree-spawner** — daemon, per-UID supervisor, PAM module, and safe
  `/etc/pam.d/system-login` integration utility.
- **usertree-spawner-backend-s6-user** — per-user s6-svscan/s6-rc backend using
  the s6-user package.
- **usertree-spawner-s6** — system s6-rc service definition. It does not select
...
1. Set `backend = "s6-user"` in `/etc/usertree-spawner/config.toml`.
2. Enable and start the system `usertree-spawnerd` service.
3. Confirm the daemon is running.
4. Run `sudo usertree-spawner-pam enable`.
