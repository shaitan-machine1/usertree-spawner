...
Builds this standalone usertree-spawner repository into ./packages.
...
rm -f -- "$package_dir"/usertree-spawner-*.pkg.tar.*
...
mapfile -t built_packages < <(
    find "$package_dir" -maxdepth 1 -type f \
        -name 'usertree-spawner-*.pkg.tar.*' ! -name '*.sig' -print | sort
)
if (( ${#built_packages[@]} == 0 )); then
    echo 'No usertree-spawner packages were produced.' >&2
    exit 1
fi

printf '\nBuilt packages:\n'
printf '  %s\n' "${built_packages[@]}"
printf '\nInstall the standalone usertree-spawner package set with:\n  sudo pacman -U'
